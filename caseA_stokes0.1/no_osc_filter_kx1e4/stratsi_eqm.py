...
'''
nz_eqm  = 1024
z_basis = de.Chebyshev('z', nz_eqm, interval=(zmin,zmax), dealias=2)
#z_basis = de.Fourier('z', nz_eqm, interval=(zmin,zmax), dealias=2)
...
problem.parameters['eps_zmax']         = epsilon(zmax)
problem.parameters['eps_zmin']         = epsilon(zmin)
problem.parameters['deps_zmax']        = depsilon(zmax)
problem.parameters['right_bc']         = epsilon(zmax)*depsilon(zmax)*stokes*zmax/(1.0+epsilon(zmax))**2
problem.parameters['dright_bc']        = stokes*(depsilon(zmax)**2*zmax*(1.0-epsilon(zmax)) + epsilon(zmax)*(d2epsilon(zmax)*zmax+depsilon(zmax))*(1.0+epsilon(zmax)))/(1.0+epsilon(zmax))**3
...
#problem.add_bc("right(vgx+eps_profile*vdx)  = 2.0*eta_hat*right_bc")
#problem.add_bc("right(dz(vgx + eps_profile*vdx))   = 2.0*eta_hat*dright_bc")
#problem.add_bc("right(vgy + eps_zmax*vdy) =-eta_hat")

#problem.add_bc("right((vgx    + eps_profile*vdx)/(1+eps_profile)) = 0.0")
#problem.add_bc("right((vgy    + eps_profile*vdy)/(1+eps_profile)) =-eta_hat")

#problem.add_bc("right(vdx-vgx)            =-2.0*eta_hat*stokes/(1.0+eps_zmax)")
#problem.add_bc("right(vdy)            = -eta_hat/(1.0+eps_zmax)")
...
out_scales = nz_vert/nz_eqm
zaxis = domain.grid(0,scales=out_scales)
vgx.set_scales(out_scales, keep_data=True)
vgy.set_scales(out_scales, keep_data=True)
vdx.set_scales(out_scales, keep_data=True)
vdy.set_scales(out_scales, keep_data=True)
