...
#waves.substitutions['gas_mass_LHS']="ikx*Ugx + dz(Ugz)"
...
for i, kx in enumerate(kx_space):
    ...
    if sigma.size > 0:
        ...
        for n, mode in enumerate(sigma_index):
            ...
            dUgz.set_scales(scales=out_scale)            
            ...
            eigenfunc['dUgz'][i].append(np.copy(dUgz['g']))           
    else:
        ...
        eigenfunc['dUgz'][i].append(np.zeros(nz_out))
...
with h5py.File('stratsi_modes.h5','w') as outfile:
    ...
    for i, freq in enumerate(eigenfreq):
        ...
        data_group.create_dataset('eig_dUgz',data=eigenfunc['dUgz'][i])
    outfile.close()
